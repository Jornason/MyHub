...
void View::initialize(int orientation)
{
  for (unsigned int i = 0; i < objectsList.size(); i++)
  {
    delete objectsList[i];
  ...
  Mesh* model = new Mesh(name, filename, orientation);
  model->setTransform(glm::mat4(1.0));
  objectsList.push_back(model);
}
void View::draw(float Xangle, float Yangle, double tolerance, double scale)
{
  ...
  for (unsigned int i = 0; i < objectsList.size(); i++)
  {
    ...
    glm::mat4 objTransf = modelview * objectsList[i]->getTransform();
    glUniformMatrix4fv(mvLoc, 1, GL_FALSE, glm::value_ptr(objTransf));
    objectsList[i]->draw(objColLoc, tolerance);
  }
  glFinish();
}
