    ...
    else
    {
        ...
        else
        {
            ...
            if ((h.time<0) || (h.time>t))
            {
                ...
                h.mat = material;
                //if(ptIntersect.y > 1 || ptIntersect.y < -1)
                //{
                //  //cout<<"y = "<<ptIntersect.y<<endl;
                //  ptIntersect.y = ptIntersect.y - (int)ptIntersect.y;
                //}
                GLfloat xTexture = 1.0f - ((glm::atan(ptIntersect.z, ptIntersect.x))/6.283185308f);
                //GLfloat xTexture = (glm::acos(ptIntersect.y)/3.141592654);
                GLfloat yTexture = 0.5 + ((glm::asin(ptIntersect.y)/6.283185308f) * 2);
                /*GLfloat yTexture;
                if(ptIntersect.x > 1 || ptIntersect.x < -1)
                {
                  cout<<"x = "<<ptIntersect.x<<endl;
                }
                if(ptIntersect.y >=0)
                {
                  yTexture = (glm::acos(ptIntersect.x/glm::sin(3.141592654*xTexture))) / 6.283185308f;
                }
                else
                {
                  yTexture = (3.141592654 + glm::acos(ptIntersect.x/glm::sin(3.141592654 *xTexture)))/ 6.283185308f;
                }*/
                if(yTexture > 1.0 || yTexture < 0.0)
                {
                  cout<<"oops"<<endl;
                }
