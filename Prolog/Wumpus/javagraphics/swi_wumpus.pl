...
:- ensure_loaded('nl_hint.pl').      % Natural language hint
...
evaluate_agent(Trials,Score,Time) :-
  run_agent_trials(Trials,1,Score,Time,Successes),
  nl,write('Number of successes: '),write(Successes),nl.
...
run_agent_trials(Trials,NextTrial,0,0,0) :-
  NextTrial > Trials.

run_agent_trials(Trials,NextTrial,Score,Time,Successes) :-
  NextTrial =< Trials,
  format("Trial ~d~n",[NextTrial]),
  %map(MAP), %determine which map to be used
  initialize(random,Percept),   %  will be modified for map maker
  ...
  run_agent_trials(Trials,NextTrial1,Score2,Time2,Successes2),
  Score is Score1 + Score2,
  Successes is Successes1 + Successes2,
...
%run_agent_trial(_,_,_) :-
 % cave(CAVE),
 % jpl_get(CAVE,'getAction',@(false)),!.
  %run_agent_trial(_,_,_).
...
run_agent_trial(NumActions,Percept,Time) :-
  ...
  start_timer,
  wait(3), 
  % get_percepts(Percept,PerceptArray),
  %jpl_call('DisplayManager','delay',[NumActions,PerceptArray],_),
  jpl_set(Cave,'getAction',@(true)),
  run_agent_trial(NumActions,Percept,Time).
...
run_agent_trial(NumActions,Percept,Time) :-
  ...
  %get_percepts(Percept,Array),
  %jpl_call('DisplayManager','printActions',[Time],_),
  %jpl_call('DisplayManager','delay',[NumActions,Array,Time],_),
  cave(CAVE),
  jpl_get(CAVE,'getAction',@(true)),
  stop_timer,
  statistics(runtime,[T2|_]),
  execute(Action,Percept1),!,
  jpl_set(CAVE,'getAction',@(false)),  
...
%
%   Size: The size of the wumpus world is fixed at 8x8, but can be set
...
initialize_world(fig62) :-
  %jpl_new('WumpusGUI',[],GUI),
  %jpl_get(GUI,'thecave',CAVE),
  %retractall(cave(_)),
  %assert(cave(CAVE)),
  ww_retractall,
...
%  random_member(Rule,Rules),
  addto_ww_init_state(wumpus_movement_rule(sit)),
...
initialize_world(random) :-
  ...
  retractall(ww_initial_state(_)),
  assert(ww_initial_state([])),
  addto_ww_init_state(wumpus_world_extent(8)),
  all_squares(8,AllSqrs),
...
% at_least_one_gold(Extent): Ensures that at least one gold piece is
...
execute(climb,[no,no,no,no,no,[],[]]) :-  % climb works, no wumpus movement
  ...
  cave(CAVE),
  jpl_call('DisplayManager','setScore',[S1],_),
  %jpl_call(CAVE,'updateScore',[],_),
  jpl_call(CAVE,'exitCave',[],_),
  jpl_call('WumpusGUI','constructNewCave',[],_),
...
get_the_gold :-
  ...
  jpl_new('java.awt.Point',[X,Y],POINT),
  jpl_call(CAVE,'grabGold',[POINT],_),
...
java_points([X|Xtail],[Y|Ytail],Points):-
jpl_new(class([java,awt],['Point']),[X,Y],POINT),
java_points(Xtail,Ytail,[POINT|Points]).




% initialize_ui: Populates the cave that is initialy display in the java GUI.
initialize_ui :-
  cave(CAVE),
  findall(PX,pit(PX,_),PXlist),
  findall(PY,pit(_,PY),PYlist),
  jpl_new(array(int),PXlist,PitXs),
  jpl_new(array(int),PYlist,PitYs),
  jpl_call('DisplayManager','setPoints',[PitXs,PitYs],PITS),
  %jpl_new('WumpusGUI',[],GUI),
  %jpl_get(GUI,'thecave',CAVE),
  jpl_call(CAVE,'drawPits',[PITS],_),
  findall(GX,gold(GX,_),GXlist),
  findall(GY,gold(_,GY),GYlist),
  jpl_new(array(int),GXlist,GoldXs),
  jpl_new(array(int),GYlist,GoldYs),
  jpl_call('DisplayManager','setPoints',[GoldXs,GoldYs],GOLD),
  jpl_call(CAVE,'drawGold',[GOLD],_),
  wumpus_location(WX,WY),
  jpl_new('java.awt.Point',[WX,WY],WLoc),
  jpl_call(CAVE,'drawWumpus',[WLoc],_).
  %retractall(cave(_)),
  %assert(cave(CAVE)).
...
update_ui :-
  ...
  jpl_call(CAVE,'updateMoveCounter',[],_),
  jpl_new('java.awt.Point',[AX,AY],ALoc),
  agent_orientation(AO),
  jpl_call(CAVE,'drawAgent',[ALoc,AO],_),
  jpl_call('DisplayManager','setAgentOrientation',[AO],_),
  %agent_in_cave(Incave),
  agent_health(Alive),
  agent_gold(HasGold),
  agent_arrows(HasArrow),
  jpl_new('[Ljava.lang.String;',['yes',Alive],StatusOne),
  jpl_new(array(int),[HasGold,HasArrow],StatusTwo),
  jpl_call('DisplayManager','setStatus',[StatusOne,StatusTwo],_),
  agent_score(Score).
...
display_action(Action,NLHint) :-
  jpl_call('DisplayManager','agentPerforms',[Action],_),
...
% succeeds P percent of the time
%
% maybe(+Probability)
...
get_percepts([St,Br,Gl,Bu,Sc|_],Array):-
  jpl_new('[Ljava.lang.String;',[St,Br,Gl,Bu,Sc],Array).


%
% Gets the cureents local time.
%  
current_time(H,M,S):-
get_time(TS),
...
seconds_elapsed(Seconds):-
 start_time(H,M,S),
 get_time(CurrTS),
 stamp_date_time(CurrTS,Date2,'local'),
 arg(4,Date2,CurrH),
 arg(5,Date2,CurrM),
 arg(6,Date2,CurrS),
 Secs2 is CurrH*360 + CurrM*60 + CurrS,
 Secs1 is H*360 + M*60 + S,
 Seconds is abs(Secs2 - Secs1).



start_timer :-
 get_time(T),
 stamp_date_time(T,Date2,'local'),
 arg(4,Date2,H),
 arg(5,Date2,M),
 arg(6,Date2,S),
 retractall(start_time(_,_,_)),
